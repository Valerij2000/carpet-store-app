...
const statuses = ['Принят', 'Собирается', 'Передан в доставку', 'Доставлен']
    ...
    <main>
      ...
      <div className="container order-page">
        ...
        <ol className="order-timeline">
          {statuses.map((status, index) => <li className={index <= currentStatus ? 'is-complete' : ''} key={status}><span>{index + 1}</span><div><strong>{status}</strong>{index === currentStatus && <p>Мы сообщим об изменении статуса заказа.</p>}</div></li>)}
        </ol>
